...
public class DemoteAdminCommand implements Command {
	...
	public Router execute(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String adminUserLogin = (String) request.getAttribute(RequestAttribute.CURRENT_USER.getValue());
			ServiceProvider serviceProvider = ServiceProvider.getInstance();
			AdminService adminService = serviceProvider.getAdminService();
			boolean result = adminService.demoteAdmin(adminLogin, adminUserLogin);
