...
/**
 * Implementer of {@link Command} interface, designed for baning user through
 * model layer.
 * 
 * @author Aliaksey Shysh
 * 
 * @see AdminService#banUser(String, String, String)
 * 
 */
	...
	public Router execute(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			String jsonResponse = new JSONObject()
					...
					.put(Feedback.Key.DAYS_TO_BAN_STATUS.getValue(), result.get(Feedback.Key.DAYS_TO_BAN_STATUS))
					...
					.put(Feedback.Key.DAYS_TO_BAN_FEEDBACK.getValue(),
							localeAttribute
									.getLocalizedMessage(result.get(Feedback.Key.DAYS_TO_BAN_FEEDBACK).toString()))
					.toString();
			...
			logger.log(Level.ERROR, "Exception occurred while user banning: {} {}", e.getMessage(), e.getStackTrace());
